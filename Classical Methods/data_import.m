...
fprintf('data importing completed!/n');
...
fprintf('/nDatasets filtering completed!/n');     
...
disp(find(recon_deg > 0.8,1));
disp(find(recon_deg > 0.9,1));
disp(find(recon_deg > 0.95,1));
...
% 
score = score(:,1:1063);
final_data = [score,label_processed];
dlmwrite('final_data.txt',final_data);

fprintf('final data generating!');



%%
data_ninty = score(:,1:441);
data_ninty = [data_ninty,final_data(:,1064)];

dlmwrite('dataNinty.txt',data_ninty);
% rng(1);
% 
